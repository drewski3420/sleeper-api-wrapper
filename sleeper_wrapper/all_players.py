"""Player collection and local cache helpers."""
...
class AllPlayers(BaseApi):
  ...
  def __init__(self, season: int, sport: str):
    ...
    Args:
      season: Season to load players for.
      sport: Sport key to load players for.
    """
    self._sport = sport
    self._season = season
  ...
  def _get_contents(self, skip_check: bool) -> dict[str, dict[str, Any]]:
    ...
    Args:
      skip_check: Whether to skip the local file existence check.
  ...
  def _populate_file(self) -> None:
    """Fetch player data from the API and write it locally."""
  ...
  def get_player(self, player_id: int | str) -> Player:
    """Build a Player for the given player id.

    Args:
      player_id: Player identifier to look up.
